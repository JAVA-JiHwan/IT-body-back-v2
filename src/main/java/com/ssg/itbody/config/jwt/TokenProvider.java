package com.ssg.itbody.config.jwt;
...
public class TokenProvider {
    ...
    public String generateAccessToken(UserEntity userEntity, Duration expiredAt){ //엑세스 토큰 생성
        // 아래 메서드 작성 완료되면 추가로 작성
        Date now = new Date();
        return makeToken(new Date(now.getTime() + expiredAt.toMillis()), userEntity);
    }

    public String generateRefreshToken(UserEntity userEntity, Duration expiredAt){ // 리프레시 토큰 생성
    ...
    private String makeToken(Date expiry, UserEntity userEntity){
        ...
        return Jwts.builder() // 빌더패턴으로 토큰 생성
                ...
                .setSubject(userEntity.getEmail()) // 기본적으로는 아이디를 줍니다
                .claim("id", userEntity.getUserId()) // 클레임에는 PK제공
                .claim("role", userEntity.getAuthority().getKey())
    ...
    public boolean validToken(String token){
        try{
            Jwts.parser() // 토큰을 먼저 파싱(읽어들임)하고
                    ...
                    .parsePlaintextJws(token); // 검증 타겟이 되는 토큰 입력
